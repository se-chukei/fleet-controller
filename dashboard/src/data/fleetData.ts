...
export function generateInitialFleet(): FleetEndpoint[] {
  ...
  for (let i = 0; i < 102; i++) {
    ...
    let activePlayer: 'PLAYER_A' | 'PLAYER_B' = 'PLAYER_A';
    let playerViewType: 'SurfaceView' | 'TextureView' = 'SurfaceView';
    ...
    let bitrateMbps = 4.2 + (Math.random() * 1.5); // Average 4-5.5 Mbps 1080p RTMP
    ...
    if (i === 12) {
      ...
      appState = 'PLAYBACK';
      usbAttached = true;
      bitrateMbps = 12.5; // high-quality local MP4
    } else if (i === 45) {
    ...
    fleet.push({
      ...
      logs: isDormant ? [
        ...
        `[${new Date().toLocaleTimeString()}] Player bound to SurfaceView overlay (hardware mode).`,
        `[${new Date().toLocaleTimeString()}] Telebeat high-frequency loop listening at interval 2s + Jitter.`
      ]
    });
  }
...
export const MOCK_GENERIC_LOGS = [
  'PLAYER_PLAYING: Buffer filled. Surface compositing locked.',
...
export const MOCK_ERROR_LOGS = [
  'PLAYER_ERROR: Socket reset by peer. Commencing local failover check.',
